...
import 'package:fayoum_club/features/news/presentation/widgets/categorized_news_tile.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:skeletonizer/skeletonizer.dart';

class RelatedNewsSection extends StatelessWidget {
  const RelatedNewsSection({super.key, required this.news});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create:
          (context) =>
              getIt<NewsCubit>()..fetchNews(
                status: news.status,
                typeOption: news.typeOption,
                activityId: news.activate!.id,
              ),
